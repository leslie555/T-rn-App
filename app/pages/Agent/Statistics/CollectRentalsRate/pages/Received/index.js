...
export default class Received extends Component {
  constructor(props) {
    ...
    this.currentCItem = {};
    this.ReceivedType = this.props.navigation.getParam("ReceivedType", "");
  ...
  render() {
    ...
    const renderItem = ({ item }) => {
      return <ListItem item={item} disabled={true} />;
    };
    return (
      <View style={styles.container}>
        <Header
          title={!this.ReceivedType ? "已收" : "今日已收"}
        ...
        <View style={styles.head_filter}>
          {!this.ReceivedType && (
            ...
            >
              ...
              <IconFont name="sanjiaoxing" size={8} color={this.state.color} />
            </TouchableOpacity>
          )}
          <TouchableOpacity
            onPress={() => {
              this._onPress("billProject");
            }}
            style={styles.select_btn}
          >
        ...
        <List
          ...
          form={this.state.form}
          listKey={`AgentReceivedList${this.type}`}
