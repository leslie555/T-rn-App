...
export default class SelectAggPayMode extends Component {
    ...
    componentWillMount() {
        ...
        if (id) {
            this.setState({
                billId: id,
                billInfo: payInfo,
                payAmount: id.length === 1?unPaidAmount:totalAmount
            })
        }
    }
    ...
    handleConfirmButton() {
        ...
        if(item.billId.length === 1) {
            // 验证支付金额
            const payMoney = item.payAmount
            const unPaidMoney = item.billInfo.unPaidAmount - 0
            if(payMoney === '') {
                toastMsg('请输入支付金额')
                return false
            }
        ...
        // debugger
        this.props.navigation.navigate('AgentAggPayPage', {
    ...
    render() {
        let WXIcon, ZFBIcon
        const item = this.state
        const isSinglePay = item.billId.length === 1
        WXIcon = item.WXFlag ? (<IconFont name='selected' size={20} color='#ff5a5a'/>) : (
            <IconFont name='unselected' size={20} color='#dddddd'/>)
        ZFBIcon = !item.WXFlag ? (<IconFont name='selected' size={20} color='#ff5a5a'/>) : (
            <IconFont name='unselected' size={20} color='#dddddd'/>)
