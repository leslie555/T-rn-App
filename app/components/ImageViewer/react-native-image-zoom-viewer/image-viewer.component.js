...
var ImageViewer = /** @class */ (function (_super) {
    ...
    ImageViewer.prototype.loadImage = function (index) {
        ...
        // 使用阿里接口或者宽高
        const apiUrl = image.url.split('?')[0]
        fetch(apiUrl + '?x-oss-process=image/info', {
            method: 'Get',
            headers: {
            ...
            if (res.status == 200) {
                ...
                imageStatus.width = +resJson.ImageWidth.value;
                imageStatus.height = +resJson.ImageHeight.value;
                imageStatus.status = 'success';
                saveImageSize();
            } else {
                throw newError('next')
            ...
            try {
                ...
                imageStatus.width = data.width || 400;
                imageStatus.height = data.height || 300;
                imageStatus.status = 'success';
                saveImageSize();
            }
            catch (newError) {
