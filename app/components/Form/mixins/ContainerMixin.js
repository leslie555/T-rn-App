...
module.exports = {
  ...
  handleFocus(scrollToTopOfWidget = false) {
    if (Platform.OS !== 'android' && this.props.scrollEnabled === true) {
      ...
      if (
        this.props.scrollOnTap === true &&
        this._pageY + this._locationY >
          Dimensions.get('window').height - keyboardHeight - 44
      ) {
  ...
  _renderContainerView() {
    ...
    var viewStyle = [
      this.props.isModal === false
        ...
        : [styles.modalView, this.props.style||{}]
    ]
    if (this.props.scrollEnabled === true) {
      return (
        <ScrollView
          ...
          onTouchStart={
            this.props.scrollOnTap === true ? this._onTouchStart : null
          }
