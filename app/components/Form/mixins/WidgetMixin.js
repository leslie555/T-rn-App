...
module.exports = {
  ...
  getDefaultValidators(defaultValidator) {
    var formState = GiftedFormManager.stores[this.props.formName]
    if (this.props.required) {
      formState.validators[this.props.name] =
        formState.validators[this.props.name] || {}
      const formStateValidator = formState.validators[this.props.name]
      formStateValidator.title = this.props.title
      if (formStateValidator.validate) {
        formStateValidator.validate.push(defaultValidator)
      } else {
        formStateValidator.validate = [defaultValidator]
      }
    }
  },
  componentDidMount() {
  ...
  componentWillUnmount() {
    if (this.props.name) {
      GiftedFormManager.deleteValue(this.props.formName, this.props.name)
    }
  },
  ...
  _renderValidationError() {
    ...
    return <ValidationErrorWidget message={this.state.validationErrorMessage} />
  },
  ...
  _renderStar() {
    return (
      <Text
        style={{
          color: 'red',
          marginLeft: this.props.required ? 10 : 16,
          marginRight: 4
        }}
      >
        {this.props.required ? '*' : ''}
