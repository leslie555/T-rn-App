...
class Manager {
  ...
  handleDeleteValue(obj) {
    this.initForm(obj.formName)
    delete this.stores[obj.formName].values[obj.name]
          ...
          for (var j in validated.results[k]) {
            if (validated.results[k].hasOwnProperty(j)) {
              if (validated.results[k][j].isValid === false) {
                let defaultMessage = !!validated.results[k][j].value
                  ? notValidMessage
                  : requiredMessage
                errors.push(
                  validated.results[k][j].message ||
                    defaultMessage.replace(
                      '{TITLE}',
                      validated.results[k][j].title
                    )
                )
